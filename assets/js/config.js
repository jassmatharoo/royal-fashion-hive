/* =========================================================
   Royal Fashion Hive — Config v3.0
...
function rfhSaveConfig(c){
  try{localStorage.setItem('rfh_config',JSON.stringify(c));return true;}catch(e){return false;}
}
function rfhResetConfig(){
  ['rfh_config','rfh_logo'].concat(Object.keys(localStorage).filter(k=>k.startsWith('rfh_img_'))).forEach(k=>localStorage.removeItem(k));
}
function rfhSaveLogo(d){try{localStorage.setItem('rfh_logo',d);return true;}catch(e){if(e.name==='QuotaExceededError')alert('Storage full! Use a smaller image (under 200KB).');return false;}}
function rfhGetLogo(){return localStorage.getItem('rfh_logo')||null;}
function rfhSaveImage(id,d){try{localStorage.setItem('rfh_img_'+id,d);return true;}catch(e){if(e.name==='QuotaExceededError')alert('Storage full! Remove some images.');return false;}}
...
/* ═══════ FIREBASE CLOUD SYNC ═══════ */
function rfhGetFirebaseUrl(){return localStorage.getItem('rfh_firebase_url')||'';}
function rfhSetFirebaseUrl(url){localStorage.setItem('rfh_firebase_url',url.replace(/\/+$/,''));}

async function rfhCloudSave(cfg){
  const url=rfhGetFirebaseUrl();if(!url)return{ok:false,err:'No Firebase URL configured'};
  try{
    // Save config (without password for security — keep that local only)
    const safeCfg=Object.assign({},cfg);
    delete safeCfg.adminPassword;
    const r=await fetch(url+'/rfh/config.json',{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(safeCfg)});
    if(!r.ok)throw new Error('HTTP '+r.status);
    return{ok:true};
  }catch(e){return{ok:false,err:e.message};}
}

async function rfhCloudLoad(){
  const url=rfhGetFirebaseUrl();if(!url)return null;
  try{
    const r=await fetch(url+'/rfh/config.json');
    if(!r.ok)return null;
    const data=await r.json();
    return data?Object.assign({},RFH_DEFAULTS,data):null;
  }catch(e){return null;}
}

async function rfhCloudSaveImage(key,dataUrl){
  const url=rfhGetFirebaseUrl();if(!url)return{ok:false,err:'No Firebase URL'};
  try{
    const r=await fetch(url+'/rfh/images/'+key+'.json',{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(dataUrl)});
    if(!r.ok)throw new Error('HTTP '+r.status);
    return{ok:true};
  }catch(e){return{ok:false,err:e.message};}
}

async function rfhCloudGetImage(key){
  const url=rfhGetFirebaseUrl();if(!url)return null;
  try{
    const r=await fetch(url+'/rfh/images/'+key+'.json');
    if(!r.ok)return null;
    const d=await r.json();return d||null;
  }catch(e){return null;}
}

async function rfhCloudDeleteImage(key){
  const url=rfhGetFirebaseUrl();if(!url)return;
  try{await fetch(url+'/rfh/images/'+key+'.json',{method:'DELETE'});}catch(e){}
}

/* Helper: get image with cloud fallback */
async function rfhGetImageWithCloud(id){
  // Try local first (faster), then cloud
  const local=rfhGetImage(id);
  if(local)return local;
  if(rfhGetFirebaseUrl()){
    const cloud=await rfhCloudGetImage('prod_'+id);
    if(cloud){rfhSaveImage(id,cloud);return cloud;} // cache locally
  }
  return null;
}
...
function rfhRecordFail(){
  const a=(parseInt(localStorage.getItem(_LA)||0))+1;localStorage.setItem(_LA,a);
  if(a>=5){localStorage.setItem(_LK,Date.now()+300000);localStorage.removeItem(_LA);}return a;
}
...
function rfhIsSessionValid(){return sessionStorage.getItem('rfh_admin')==='1'&&Date.now()<parseInt(sessionStorage.getItem(_SE)||0);}
...
function rfhSanitize(s){const m={'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#x27;'};return String(s||'').replace(/[&<>"']/g,c=>m[c]);}
...
function rfhPasswordStrength(p){
  ...
  if(s<=1)return{level:'weak',label:'Weak',color:'#e53935'};if(s<=2)return{level:'fair',label:'Fair',color:'#fb8c00'};
  if(s<=3)return{level:'good',label:'Good',color:'#fdd835'};return{level:'strong',label:'Strong',color:'#43a047'};
}
